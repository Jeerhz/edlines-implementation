#include "original_EDLines.h"
#include "../NFA.h"

using namespace cv;
using namespace std;

OriginalEDLines::OriginalEDLines(Mat srcImage, double _line_error, int _min_line_len, double _max_distance_between_two_lines, double _max_error)
	: OriginalED(srcImage, SOBEL_OPERATOR, 36, 8)
{
...
#undef PRECISON_ANGLE
	...
	for (int i = 0; i < linesNo; i++)
	{
		...
		linePoints.push_back(OriginalLS(start, end));
	} // end-for
...
OriginalEDLines::OriginalEDLines(OriginalED obj, double _line_error, int _min_line_len, double _max_distance_between_two_lines, double _max_error)
	: OriginalED(obj)
{
	min_line_len = _min_line_len;
	line_error = _line_error;
...
OriginalEDLines::OriginalEDLines()
{
	//
}

vector<OriginalLS> OriginalEDLines::getLines()
{
	return linePoints;
}

int OriginalEDLines::getLinesNo()
{
	return linesNo;
}

Mat OriginalEDLines::getLineImage()
{
...
Mat OriginalEDLines::drawOnImage()
{
...
int OriginalEDLines::ComputeMinLineLength()
{
...
//
void OriginalEDLines::SplitSegment2Lines(double *x, double *y, int noPixels, int segmentNo)
{
	...
	while (noPixels >= min_line_len)
	{
		...
		while (index < noPixels)
		{
			...
			if (goodPixelCount < 2 || index >= noPixels)
			{
				...
				lines.push_back(OriginalLineSegment(lastA, lastB, lastInvert, sx, sy, ex, ey, segmentNo, firstPixelIndex + noSkippedPixels, index - noSkippedPixels + 1));
				linesNo++;
				len = index + 1;
				break;
			} // end-else
...
//
void OriginalEDLines::JoinCollinearLines()
{
...
void OriginalEDLines::ValidateLineSegments()
{
	...
	for (int i = 0; i < linesNo; i++)
	{
		OriginalLineSegment *ls = &lines[i];
...
double OriginalEDLines::ComputeMinDistance(double x1, double y1, double a, double b, int invert)
{
	double x2, y2;
...
//
void OriginalEDLines::ComputeClosestPoint(double x1, double y1, double a, double b, int invert, double &xOut, double &yOut)
{
	double x2, y2;
...
//
void OriginalEDLines::LineFit(double *x, double *y, int count, double &a, double &b, int invert)
{
	if (count < 2)
		return;
...
// Fits a line of the form y=a+bx (invert == 0) OR x=a+by (invert == 1)
//
void OriginalEDLines::LineFit(double *x, double *y, int count, double &a, double &b, double &e, int &invert)
{
	if (count < 2)
		return;
...
//
bool OriginalEDLines::TryToJoinTwoLineSegments(OriginalLineSegment *ls1, OriginalLineSegment *ls2, int changeIndex)
{
	int which;
	...
	OriginalLineSegment *shorter = ls1;
	OriginalLineSegment *longer = ls2;
...
//
double OriginalEDLines::ComputeMinDistanceBetweenTwoLines(OriginalLineSegment *ls1, OriginalLineSegment *ls2, int *pwhich)
{
...
//
void OriginalEDLines::UpdateLineParameters(OriginalLineSegment *ls)
{
...
void OriginalEDLines::EnumerateRectPoints(double sx, double sy, double ex, double ey, int ptsx[], int ptsy[], int *pNoPoints)
{
