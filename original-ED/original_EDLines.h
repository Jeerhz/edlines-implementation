...
struct OriginalLS
{
	...
	OriginalLS(cv::Point2d _start, cv::Point2d _end)
	{
		start = _start;
		end = _end;
	}
...
struct OriginalLineSegment
{
	...
	OriginalLineSegment(double _a, double _b, int _invert, double _sx, double _sy, double _ex, double _ey, int _segmentNo, int _firstPixelIndex, int _len)
	{
		a = _a;
		b = _b;
		invert = _invert;
...
class OriginalEDLines : public OriginalED
{
public:
	OriginalEDLines(cv::Mat srcImage, double _line_error = 1.0, int _min_line_len = -1, double _max_distance_between_two_lines = 6.0, double _max_error = 1.3);
	OriginalEDLines(OriginalED obj, double _line_error = 1.0, int _min_line_len = -1, double _max_distance_between_two_lines = 6.0, double _max_error = 1.3);
	OriginalEDLines();

	std::vector<OriginalLS> getLines();
	int getLinesNo();
	cv::Mat getLineImage();
	cv::Mat drawOnImage();

private:
	std::vector<OriginalLineSegment> lines;
	std::vector<OriginalLineSegment> invalidLines;
	std::vector<OriginalLS> linePoints;
	int linesNo;
	int min_line_len;
	double line_error;
	...
	bool ValidateLineSegmentRect(int *x, int *y, OriginalLineSegment *ls);
	bool TryToJoinTwoLineSegments(OriginalLineSegment *ls1, OriginalLineSegment *ls2, int changeIndex);
	...
	static double ComputeMinDistanceBetweenTwoLines(OriginalLineSegment *ls1, OriginalLineSegment *ls2, int *pwhich);
	static void UpdateLineParameters(OriginalLineSegment *ls);
