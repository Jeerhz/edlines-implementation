#include "original_ED.h"
#include <fstream>

using namespace cv;
using namespace std;

OriginalED::OriginalED(Mat _srcImage, GradientOperator _op, int _gradThresh, int _anchorThresh, int _scanInterval, int _minPathLen, double _sigma, bool _sumFlag)
{
...
OriginalED::OriginalED(const OriginalED &cpyObj)
{
...
OriginalED::OriginalED(short *_gradImg, uchar *_dirImg, int _width, int _height, int _gradThresh, int _anchorThresh, int _scanInterval, int _minPathLen, bool selectStableAnchors)
{
	height = _height;
	width = _width;
...
OriginalED::OriginalED()
{
	//
}

Mat OriginalED::getEdgeImage()
{
	return edgeImage;
}

Mat OriginalED::getAnchorImage()
{
...
Mat OriginalED::getSmoothImage()
{
	return smoothImage;
}

Mat OriginalED::getGradImage()
{
	Mat result8UC1;
...
int OriginalED::getSegmentNo()
{
	return segmentNos;
}

int OriginalED::getAnchorNo()
{
	return anchorNos;
}

std::vector<Point> OriginalED::getAnchorPoints()
{
	return anchorPoints;
}

std::vector<std::vector<Point>> OriginalED::getSegments()
{
	return segmentPoints;
}

std::vector<std::vector<Point>> OriginalED::getSortedSegments()
{
...
Mat OriginalED::drawParticularSegments(std::vector<int> list)
{
...
void OriginalED::ComputeGradient()
{
	...
	for (int i = 1; i < height - 1; i++)
	{
		for (int j = 1; j < width - 1; j++)
		{
			...
			{
				if (gx >= gy)
					dirImg[index] = ORIGINAL_EDGE_VERTICAL;
				else
					dirImg[index] = ORIGINAL_EDGE_HORIZONTAL;
			} // end-if
		} // end-for
	} // end-for
}

void OriginalED::ComputeAnchorPoints()
{
	...
	for (int i = 2; i < height - 2; i++)
	{
		...
		for (int j = start; j < width - 2; j += inc)
		{
			...
			if (dirImg[i * width + j] == ORIGINAL_EDGE_VERTICAL)
			{
				// vertical edge
				int diff1 = gradImg[i * width + j] - gradImg[i * width + j - 1];
				int diff2 = gradImg[i * width + j] - gradImg[i * width + j + 1];
...
void OriginalED::JoinAnchorPointsUsingSortedAnchors()
{
	int *chainNos = new int[(width + height) * 8];

	Point *pixels = new Point[width * height];
	OriginalStackNode *stack = new OriginalStackNode[width * height];
	OriginalChain *chains = new OriginalChain[width * height];
	...
	StartOfWhile:
		while (top >= 0)
		{
			...
			if (dir == ED_LEFT)
			{
				while (dirImg[r * width + c] == ORIGINAL_EDGE_HORIZONTAL)
				{
			...
			{
				while (dirImg[r * width + c] == ORIGINAL_EDGE_VERTICAL)
				{
					edgeImg[r * width + c] = EDGE_PIXEL;
		...
		else
		{
			...
			if (totalLen > 0)
			{
				...
				for (int k = count - 1; k >= 0; k--)
				{
					...
					if (segmentPoints[segmentNos].size() < 2)
						continue;
...
void OriginalED::sortAnchorsByGradValue()
{
...
int *OriginalED::sortAnchorsByGradValue1()
{
	int SIZE = 128 * 256;
	int *C = new int[SIZE];
...
int OriginalED::LongestChain(OriginalChain *chains, int root)
{
...
int OriginalED::RetrieveChainNos(OriginalChain *chains, int root, int chainNos[])
{
	int count = 0;
