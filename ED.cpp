...
{
    sort(segmentPoints.begin(), segmentPoints.end(), [](const vector<Point> &a, const vector<Point> &b)
...
void ED::ComputeAnchorPoints()
{
    ...
    anchorNb = (int)anchorPoints.size();
    DEBUG_LOG("Found " << anchorNb << " anchor points");
}
...
PPoint ED::getPoint(int offset)
{
    ...
    return PPoint(row, col, grad_orientation, is_anchor, is_edge);
}
...
void ED::JoinAnchorPointsUsingSortedAnchors()
{
    ...
    for (int k = anchorNb - 1; k >= 0; k--)
    {
        ...
        ChainNode *first_chain = chain.createNewChain(anchor.grad_orientation == EDGE_VERTICAL ? UP : LEFT);
        chain.addPixelToChain(first_chain, anchor);
        ...
        while (!process_stack.empty())
        {
            ...
            ChainNode *current_chain = chain.createNewChain(currentNode.node_direction);
            exploreChain(currentNode, current_chain);
        }

        ChainNode *chain_head = chain.getFirstChain();
        vector<Point> segment = chain.extractSegmentPixels(chain_head, minPathLen);
...
StackNode ED::getNextNode(StackNode &current_node)
{
    ...
    // Check for edge/anchor pixels in 3-connected neighbors
    ...
    // No edge pixel found, follow highest gradient
...
void ED::exploreChain(StackNode &current_node, ChainNode *current_chain)
{
    ...
    // Trace chain while gradient orientation matches direction
    while (true)
    {
    ...
    if (is_horizontal)
    {
        ...
        if (current_node.node_row - 1 >= 0)
            process_stack.push_back(StackNode(current_node.node_row - 1, current_node.node_column, UP, EDGE_VERTICAL));
        if (current_node.node_row + 1 < image_height)
            process_stack.push_back(StackNode(current_node.node_row + 1, current_node.node_column, DOWN, EDGE_VERTICAL));
    }
    else
    {
        // Add LEFT and RIGHT for vertical chains
        if (current_node.node_column - 1 >= 0)
            process_stack.push_back(StackNode(current_node.node_row, current_node.node_column - 1, LEFT, EDGE_HORIZONTAL));
        if (current_node.node_column + 1 < image_width)
            process_stack.push_back(StackNode(current_node.node_row, current_node.node_column + 1, RIGHT, EDGE_HORIZONTAL));
    }
