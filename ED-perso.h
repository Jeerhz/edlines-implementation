...
struct StackNode
{
    int node_row, node_column; // starting pixel (row, column)
...
public:
    ED(cv::Mat _srcImage, GradientOperator _op = SOBEL_OPERATOR, int _gradThresh = 20, int _anchorThresh = 0, int _scanInterval = 1, int _minPathLen = 10, double _sigma = 1.0, bool _sumFlag = true);
    ED(const ED &cpyObj);
